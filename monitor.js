...
function scheduleGc() {
  ...
  var nextMinutes = Math.random() * 20 + 10;
            ...
            if (productToCompare.indexOf(configuration.keywords[x].toLowerCase()) > -1) {
              ...
              if (possibleMatch.length === 0) {
                ...
                if (possibleRestock === undefined) {
                  ...
                  // compare stock
                  if (possibleRestock.stock === 'Unavailable') {} else {
        ...
        diff.forEach(function(part) {
          ...
          if (part.added) {
            ...
            for (var i = 0; i < diffAdded.length; i++) {
              item = _.where(parsedOG, {name: diffAdded[i].name})
              if (item.length === 0) {
            ...
            for (var i = 0; i < diffRemoved.length; i++) {
              item = _.where(parsedNew, {name: diffRemoved[i].name})
